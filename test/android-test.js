...
async function main() {
  ...
  try {
    ...
    console.log('   client.call("isChannelUrl", "https://youtube.com/@test")');
    const isChannelLocal = await client.call('isChannelUrl', 'https://youtube.com/@test');
    console.log(`   ${isChannelLocal.success ? '✅' : '❌'} Success: ${isChannelLocal.success}`);
    console.log(`   Result: ${JSON.stringify(isChannelLocal.result)}`);
    console.log(`   Result type: ${typeof isChannelLocal.result}`);

    console.log('\n   client.call("getHome")');
    const homeLocal = await client.call('getHome');
    console.log(`   ${homeLocal.success ? '✅' : '❌'} Success: ${homeLocal.success}`);
    if (homeLocal.success && homeLocal.result) {
      const result = homeLocal.result;
      console.log(`   Result type: ${Array.isArray(result) ? 'Array' : result.constructor.name}`);
      ...
      if (videos && videos.length > 0) {
        ...
        console.log(`      Name: ${firstVideo.name || firstVideo.title || 'Unknown'}`);
        console.log(`      Duration: ${firstVideo.duration}s`);
        console.log(`      Playback time: ${firstVideo.playbackTime}`);
        if (firstVideo.thumbnails) {
          console.log(`      Thumbnail: ${firstVideo.thumbnails.sources?.[0]?.url || JSON.stringify(firstVideo.thumbnails).substring(0, 80)}`);
        }
      }
    }
    ...
    console.log('   ℹ️  Note: This requires an active GrayJay app connection\n');

    console.log('   client.testAndroid("isChannelUrl", "https://youtube.com/@test")');
    const isChannelAndroid = await client.testAndroid('isChannelUrl', 'https://youtube.com/@test');
    console.log(`   ${isChannelAndroid.success ? '✅' : '❌'} Success: ${isChannelAndroid.success}`);
    console.log(`   Result: ${JSON.stringify(isChannelAndroid.result)}`);

    console.log('\n   client.testAndroid("getHome")');
    const homeAndroid = await client.testAndroid('getHome');
    console.log(`   ${homeAndroid.success ? '✅' : '❌'} Success: ${homeAndroid.success}`);
    if (homeAndroid.success && homeAndroid.result) {
      ...
      if (videos && videos.length > 0) {
        ...
        console.log(`      Playback time: ${firstVideo.playbackTime}`);
        if (firstVideo.thumbnails) {
          console.log(`      Thumbnails: ${JSON.stringify(firstVideo.thumbnails).substring(0, 150)}...`);
        }
      }
    }
